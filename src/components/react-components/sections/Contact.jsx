...
export default function Contact({ urls }) {
  ...
  const submitForm = (ev) => {
    ...
    if (lastDate && newDate - lastDate < HALF_HOUR) {
      return toast.error(
        "🕓 Você já enviou uma mensagem recentemente, aguarde mais tarde..."
      );
    }

    if (document.contactForm.length !== 5)
    ...
    for (let i = 0; i < document.contactForm.length - 1; i++) {
      ...
      if (document.contactForm[3].value.length < 20) {
        ...
        return toast.error(
          "Essa mensagem é muito curta! Tente enviar outra..."
        );
      }
    }

    if (document.contactForm === "") setLoading(true);
      ...
      <div className="c-col" style={{ margin: "auto" }}>
        ...
        <div>
          <p className="contact-info">
            Se você quer entrar em contato comigo, conversar sobre algum projeto
            ou só bater um papo, preencha o formulário abaixo ou então entre em
            contato com alguma dos métodos abaixo :D
          </p>
        </div>
        <form
